...
namespace Kweek
{
    ...
    public class BasicMeleeAttackState : IAbstractTargetState
    {
        ...
        [SerializeField]
        private LayerMask m_RadiusLayerMask = default(LayerMask);

        [SerializeField]
        private LayerMask m_RaycastLayerMask = default(LayerMask);

        [Space(10)]
        [Header("Other States")]
        [Space(5)]
        [SerializeField]
        private IAbstractState m_WanderState = null;
        ...
        {
            Collider[] colliders = Physics.OverlapSphere(m_ViewTransform.position, m_ViewRadius, m_RadiusLayerMask.value);
            ...
            for (int i = 0; i < colliders.Length; ++i)
            {
                ...
                if (damageableObject == m_Target)
                {
                    ...
                    if (degAngle <= m_ViewAngle)
                    {
                        ...
                        Ray ray = new Ray(m_ViewTransform.position, (damageableObject.GetPosition() - m_ViewTransform.position) * 100);
