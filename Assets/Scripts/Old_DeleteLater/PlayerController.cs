...
public class PlayerController : MonoBehaviour, IMoveableObject
{
    ...
    private void HorizontalCheck()
    {
        ...
        ContactPoint[] contactPoints = m_LastCollision.contacts;
        if (contactPoints.Length == 0) { return; }
        m_LastCollision = null;

        //Filter out all the useless contact points, keeping only those with unique normals
        List<ContactPoint> filteredContactPoints = new List<ContactPoint>();
        for (int i = 0; i < contactPoints.Length; ++i)
        {
            ContactPoint currentCP = contactPoints[i];

            bool unique = true;
            for (int j = 0; j < filteredContactPoints.Count; ++j)
            {
                ContactPoint otherCP = filteredContactPoints[j];

                if (currentCP.normal == otherCP.normal)
                {
                    unique = false;
                    break;
                }
            ...
            if (unique)
            {
                filteredContactPoints.Add(currentCP);
                Debug.DrawLine(currentCP.point, currentCP.point + currentCP.normal * 2.0f, Color.red, 5.0f);
            }
        }

        if (filteredContactPoints.Count == 0)
            return;

        float lengthInWall = 0.5f;

        //If there are multiple unique normals. Don't slide
        if (filteredContactPoints.Count >= 2)
        {
            //Move backwards a bit
            transform.position -= (normDir * lengthInWall);
            return;
        ...
        //If there is only 1 unique normal, start sliding!
        Vector3 perpendicular = new Vector3(-filteredContactPoints[0].normal.z, 0.0f, filteredContactPoints[0].normal.x);
        perpendicular.Normalize();
        ...
        //Vector3 newPos = filteredContactPoints[0].point + (filteredContactPoints[0].normal * (m_Collider.radius + (m_CurrentSpeed * Time.deltaTime)));
            ...
            if (raycastHits.Length > 0)
            {
                for (int j = 0; j < raycastHits.Length; ++j)
                {
                    if (raycastHits[j].collider.gameObject != gameObject &&
                        raycastHits[j].collider.gameObject.tag != "NoPlayerCollision")
                    {
